...
//Crear instancia
Modbus slave(ID, Serial, 4); //ID del nodo. 0 para el master, 1-247 para esclavo
...
void io_setup() {
  ...
  digitalWrite(8, LOW );
  digitalWrite(9, LOW );
  digitalWrite(LED_BUILTIN, LOW ); //Led del pin 13 de la placa
...
void io_poll() {
  ...
  digitalWrite( LED_BUILTIN, bitRead( au16data[1], 4 ));
  ...
  au16data[4] = 221;//analogRead( 0 ); //El valor analógico leido en el pin A0 se guarda en au16data[4]. (siendo 0=0v y 1023=5v)
  au16data[5] = 200;//analogRead( 1 );
